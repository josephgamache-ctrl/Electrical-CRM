-- Migration: Add Time Tracking System
-- Created: 2025-12-06
...
-- TIME ENTRIES TABLE
-- ============================================================
-- Main table for tracking employee hours on jobs
-- Features:
-- - Day-level granularity (one entry per employee per job per day)
-- - Editable until end of week (Sunday 11:59 PM)
-- - Dual rate system: billable (customer) vs pay (employee)
...
CREATE TABLE IF NOT EXISTS time_entries (
    ...
    -- Week locking (entries locked after Sunday 11:59 PM of their week)
    week_ending_date DATE NOT NULL,  -- The Sunday of the week this entry belongs to
    is_locked BOOLEAN DEFAULT FALSE, -- Locked for payroll processing

    -- Optional fields
    notes TEXT,  -- Employee notes about the work performed
    ...
    -- Prevent duplicate entries (one entry per employee per job per day)
...
-- Composite index for fast "my timecard for this week" queries
CREATE INDEX idx_time_entries_employee_week_date ON time_entries(employee_username, week_ending_date, work_date);

-- ============================================================
-- EMPLOYEE PAY RATES TABLE (For Future Accounting)
-- ============================================================
-- Tracks employee pay rates over time with effective dates
-- Allows for raises and rate changes with historical tracking
...
CREATE TABLE IF NOT EXISTS employee_pay_rates (
    ...
    overtime_rate DECIMAL(10, 2),  -- Typically 1.5x hourly_rate
    ...
    job_title VARCHAR(100),  -- "Apprentice", "Journeyman", "Master Electrician"
    rate_type VARCHAR(50) DEFAULT 'hourly',  -- hourly, salary, contract
    ...
    -- Ensure no overlapping date ranges for same employee
    EXCLUDE USING gist (
...
-- ============================================================
-- JOB BILLING RATES TABLE (For Future Accounting)
-- ============================================================
-- Tracks billable rates for different job types or customers
-- Allows custom rates per customer/job with defaults

CREATE TABLE IF NOT EXISTS job_billing_rates (
    id SERIAL PRIMARY KEY,

    -- Rate scope (can be default, customer-specific, or job-type-specific)
    rate_name VARCHAR(100) NOT NULL,  -- "Standard", "Emergency", "Commercial", "Residential"
...
CREATE INDEX idx_job_billing_rates_customer ON job_billing_rates(customer_name);
...
BEGIN
    ...
    -- If input_date is Sunday, returns that date
...
-- This should be run as a scheduled job (cron) or called manually
CREATE OR REPLACE FUNCTION lock_completed_weeks()
RETURNS INTEGER AS $$
DECLARE
    rows_locked INTEGER;
BEGIN
    -- Lock all time entries for weeks that have ended (Sunday has passed)
    UPDATE time_entries
    SET is_locked = TRUE
    WHERE is_locked = FALSE
      AND week_ending_date < CURRENT_DATE
      AND EXTRACT(DOW FROM CURRENT_DATE) = 1;  -- Run on Mondays
...
CREATE OR REPLACE FUNCTION get_job_billable_rate(job_type_param VARCHAR, customer_name_param VARCHAR)
RETURNS DECIMAL AS $$
DECLARE
...
BEGIN
    ...
    WHERE is_active = TRUE
      ...
      AND (effective_to IS NULL OR effective_to >= CURRENT_DATE)
      AND (
          (rate_type = 'customer' AND customer_name = customer_name_param)
...
BEGIN
    ...
    -- ============================================================
    -- EXAMPLE SEED DATA FOR EMPLOYEE PAY RATES
    -- ============================================================
...
GROUP BY wo.id, wo.work_order_number, wo.customer_name, wo.job_type, wo.status
ORDER BY wo.work_order_number;

-- View: Daily time entry detail
...
--
-- Week Locking System:
-- - Employees can edit time entries until Sunday 11:59 PM of that week
-- - On Monday, run: SELECT lock_completed_weeks(); to lock previous week
-- - Locked entries cannot be modified (enforced by trigger)
-- - Admins can manually unlock if needed: UPDATE time_entries SET is_locked = FALSE WHERE id = ?
--
-- Rate Lookup:
-- - Pay rates: SELECT get_current_pay_rate('username');
-- - Billable rates: SELECT get_job_billable_rate('job_type', 'customer_name');
--
-- Common Queries:
-- - My timecard this week: SELECT * FROM daily_time_detail WHERE employee_username = '<username>' AND week_ending_date = calculate_week_ending(CURRENT_DATE);
-- - Weekly summary: SELECT * FROM weekly_timecard_summary WHERE employee_username = '<username>';
-- - Job cost analysis: SELECT * FROM job_labor_summary WHERE work_order_id = 123;
--
