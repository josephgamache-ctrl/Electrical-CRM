...
SELECT
    ...
    -- Profit Calculations
    wo.total_amount - COALESCE(SUM(jmu.quantity * jmu.unit_cost), 0) - COALESCE(SUM(te.pay_amount), 0) as gross_profit,
    CASE
        WHEN wo.total_amount > 0 THEN
            ((wo.total_amount - COALESCE(SUM(jmu.quantity * jmu.unit_cost), 0) - COALESCE(SUM(te.pay_amount), 0)) / wo.total_amount * 100)
        ELSE 0
...
SELECT
    ...
    COALESCE(SUM(CASE
        WHEN jmu.assigned_at > CURRENT_DATE - INTERVAL '90 days'
        THEN jmu.quantity
...
BEGIN
    ...
    WHERE jmu.assigned_at::DATE BETWEEN p_start_date AND p_end_date
